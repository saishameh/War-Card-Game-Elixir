defmodule WarTest do
  ...
  #Elena Test Cases
  test "deal_1" do
    t1 = [11,9,3,4,8,2,10,5,13,4,2,2,5,4,1,10,12,1,8,12,5,11,7,6,3,11,6,2,1,4,7,8,10,13,13,9,1,5,6,7,3,9,9,10,12,13,11,8,3,6,12,7]
    r1 = [4,2,11,8,12,11,10,7,7,5,5,3,1,10,10,9,7,6,12,9,1,13,10,9,1,11,6,6,5,4,1,12,4,3,13,3,6,2,13,8,5,2,13,8,4,2,12,8,11,7,9,3]
    assert War.deal(t1) == r1
  end

  test "deal_2" do
    t2 = [11,1,11,7,12,1,8,1,9,12,7,8,10,3,7,3,10,9,2,13,12,8,10,4,4,3,2,13,10,9,6,13,12,7,11,4,6,11,9,5,6,3,1,5,2,4,2,13,6,5,5,8]
    r2 = [3,2,7,5,12,9,9,5,11,10,10,4,9,8,1,8,13,3,1,11,12,10,1,11,1,8,13,6,6,4,12,3,11,8,10,7,13,6,6,4,4,3,2,2,13,7,12,5,9,5,7,2]
    assert War.deal(t2) == r2
  end

  test "deal_3" do
    t3 = [12,3,9,10,1,11,4,11,3,13,11,10,13,4,6,10,8,7,8,6,1,6,7,12,2,7,13,8,8,11,1,5,2,4,9,9,6,12,3,12,5,3,4,2,13,2,5,1,5,7,10,9]
    r3 = [12,12,10,9,7,5,7,3,7,2,8,6,4,3,1,3,10,2,13,11,13,9,8,8,6,4,12,10,1,5,12,4,6,3,11,7,8,2,5,2,13,10,11,9,13,9,11,5,1,6,1,4]
    assert War.deal(t3) == r3
  end

  test "deal_4" do
    t4 = [6,12,7,7,8,8,2,3,3,4,10,5,12,6,5,10,2,2,12,10,3,9,8,5,6,11,5,9,1,4,4,6,12,8,9,7,13,10,13,13,1,11,11,1,4,13,11,9,7,1,2,3]
    r4 = [8,2,8,6,5,2,3,2,1,4,12,3,11,4,7,2,1,12,1,1,9,9,8,6,13,12,11,11,9,8,6,4,13,10,12,10,13,10,10,9,13,7,6,5,11,7,7,5,5,4,3,3]
    assert War.deal(t4) == r4
  end

  test "deal_5" do
    t5 = [12,6,13,11,9,5,11,7,7,10,6,10,3,11,5,4,9,3,2,1,9,8,1,5,10,2,9,3,1,13,4,4,7,11,2,3,8,10,6,1,5,4,6,12,8,12,2,13,8,12,13,7]
    r5 = [8,7,7,5,11,4,10,2,13,9,11,11,8,8,7,3,6,5,5,2,13,10,12,5,12,9,9,2,1,13,1,3,13,12,12,2,8,6,6,4,4,4,1,1,11,7,3,3,10,10,9,6]
    assert War.deal(t5) == r5
  end
  #Given Additonal Test Cases
  test "deal_6" do
  ...
  #5 More Test Cases
  test "deal_12" do
    t12 = [11,1,2,13,10,5,8,6,12,5,13,1,4,12,13,3,13,12,11,9,7,11,11,2,10,3,1,4,2,9,6,8,10,8,7,12,5,1,9,10,7,3,8,2,3,6,4,5,4,9,7,6]
    r12 = [7,6,13,10,1,3,4,2,1,12,12,11,5,2,13,2,13,12,12,10,9,8,8,5,13,4,6,3,1,11,4,4,3,2,11,9,11,8,7,6,9,6,1,10,8,7,5,5,10,7,9,3]
    assert War.deal(t12) == r12
  end

  test "deal_13" do
    t13 = [8,13,4,10,5,8,5,12,2,1,6,4,7,9,8,7,12,3,10,13,1,8,10,12,4,1,11,2,13,6,4,5,1,11,12,6,5,3,10,13,2,9,3,7,11,9,9,11,6,7,2,3]
    r13 = [1,12,1,6,1,11,13,5,13,7,13,6,12,3,10,2,4,2,13,8,9,2,11,10,9,5,11,5,7,4,9,8,5,3,12,3,9,2,8,4,12,10,7,6,1,8,10,7,11,6,4,3]
    assert War.deal(t13) == r13
  end

  test "deal_14" do
    t14 = [13,2,6,3,6,7,1,12,7,5,8,9,8,2,11,5,3,9,8,3,4,6,5,12,7,4,10,1,13,1,4,13,2,9,7,8,11,10,6,12,12,11,10,3,5,13,11,2,4,10,1,9]
    r14 = [1,11,13,5,13,10,11,2,12,8,10,6,12,6,11,2,1,1,10,9,7,7,3,2,1,13,13,12,12,9,9,7,9,5,8,5,8,5,6,4,11,4,7,4,6,3,4,3,10,3,8,2]
    assert War.deal(t14) == r14
  end

  test "deal_15" do
    t15 = [7,10,1,12,3,11,4,6,8,8,12,10,13,7,13,11,11,1,11,4,10,2,3,9,4,13,8,9,3,6,6,1,5,10,2,5,6,8,2,7,9,2,5,5,12,4,9,3,12,7,1,13]
    r15 = [13,12,12,8,9,3,13,10,12,8,13,10,1,7,11,5,5,4,3,3,11,9,1,1,6,6,6,2,10,5,10,3,9,6,4,2,1,5,13,2,12,4,11,2,11,8,8,7,9,7,7,4]
    assert War.deal(t15) == r15
  end

  test "deal_16" do
    t16 = [9,1,4,11,10,8,6,3,6,12,4,5,8,5,13,3,3,5,4,2,2,11,10,8,4,10,1,13,7,7,11,3,13,6,10,12,6,12,7,5,9,13,2,1,11,12,7,2,9,1,9,8]
    r16 = [9,6,1,10,13,6,12,7,11,4,1,3,10,2,13,11,12,6,7,5,13,5,12,12,8,4,3,3,10,7,8,2,6,5,1,4,8,3,5,2,13,7,9,2,1,11,9,9,8,4,11,10]
